			...
			<div id="header">
				<div className="top" ref={this.topBar}>
					<div className="row flex">
						...
						<div className="col ph item s6 m1 l2"></div>
						<div className="col ph item s6 m2 l2">
							<i className="fa fa-phone color-primary"></i>
							<div>
						...
						<div className="col address item s6 m3 l2">
							<i className="fa fa-map-marker color-primary"></i>
							<div>
								<div className="prop color-primary">Address</div>
								<div dangerouslySetInnerHTML={{ __html: marked(baseData.allBaseYaml.edges[0].node.data.address.replace(/(?:\r\n|\r|\n)/g, '<br/>'))}}></div>
							</div>
						</div>
						<div className="col address item s6 m3 l2">
							<i className="fa fa-envelope color-primary"></i>
							<div>
								<div className="prop color-primary">Mail</div>
								<div><a className="text-primary" href={"mailto:"+baseData.allBaseYaml.edges[0].node.data.email}>{baseData.allBaseYaml.edges[0].node.data.email}</a></div>
							</div>
						</div>
					</div>
				</div>
				<nav className="bg-primary" ref={this.nav}>
					...
					<div className="mobile">
						<a href="#menu"  onClick={this.menuOpen}><i className="fa fa-bars"></i></a>
					</div>
					<div className="nav">
						<Navlinks />
					</div>
				...
				<Sidebar
			        sidebar={
						<SidebarContents data={baseData.allBaseYaml.edges[0].node.data}/>
					}
		...
		<div className="sidebar-contents" id="menu">
			...
			<div className="contact">
				<div className="col">
					...
					<div dangerouslySetInnerHTML={{ __html: data.phone.join("<br/>")}}></div>
				</div>
				<div className="col">
					<i className="fa fa-map-marker color-primary"></i>
					<p className="prop color-primary">Address</p>
					<div dangerouslySetInnerHTML={{ __html: marked(data.address.replace(/(?:\r\n|\r|\n)/g, '<br/>'))}}></div>
				</div>
				<div className="col">
					<i className="fa fa-envelope color-primary"></i>
					<p className="prop color-primary">Mail</p>
					<div>
						<a className="text-primary" href={"mailto:"+data.email}>{data.email}</a>
					</div>
				</div>
			</div>
		</div>
